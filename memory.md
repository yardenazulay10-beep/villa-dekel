...
## 2026-09-24: fresh-eyes review, what was removed and why
- **Testimonial "משפחת כהן · חנוכה 2024 · ציון 10/10" removed.** Yarden confirmed it was fake. It was live on a commercial booking page.
- **AggregateRating (10/10, 50 reviews) removed from the JSON-LD**, and the mobile-only stats strip that carried the "10/10 Booking.com" figure was removed entirely; its other two numbers already sit in the facts bar. The count could not be verified (Booking.com blocks fetches; Tavily quota exhausted for the month). The OneDrive fork had already made this call in April ("Remove all Booking.com score references", 8cd63b3) and it was never ported to the canonical repo. If the real Booking.com numbers are ever confirmed, the rating can come back, but only with the real count and only if a rating is visible on the page.
- **Urgency banner "🔥 הקיץ מתמלא מהר" removed.** Stale in September, contradicted the all-dates-free calendar, and was the exact loud-CTA style the brief says to beat.
- **Gallery cut 56 → 16.** The library is three tiers: the original professional shoot (bright, wide, landscape, img34-img63), Snir's video stills (cinematic, darker, portrait, img64-img83), and phone snaps (the rest). The professional shoot now leads. See [[lessons-learned]] 2026-09-24 for the mistake this corrected.
- **66 unreferenced media files deleted** from `public/media/`. 19 remain, every one referenced from `app/`: 16 gallery photos, img9 (CTA background), video7.mp4 and its poster. Recoverable from git history.
- **Schema image list**: img63 (the frame with a "HEATED POOL" text overlay baked in) swapped for img42.
- Calendar day buttons raised from 36px to 44px. `sitemap.ts` now lists only the homepage; the four legal pages are noindex and each carries its own canonical instead. Real Hebrew alt text on every gallery image via a `CAPTIONS` map in `app/page.tsx`.
- The browser pane could not render screenshots this session (window behind another), so visual checks were done from the DOM and built HTML, not from pictures. Look at the trimmed gallery on a phone before trusting the feel of it.

### Gotchas found 2026-09-24
- **`npm run build` fails while `next dev` is running**, with twenty copies of `Error while looking up import map: next/font/google queries have exactly one entry`. Nothing is wrong with the fonts; Turbopack's build and the dev server collide on the same `.next`. Stop the preview server, then build.
- **Dev console shows "Encountered a script tag while rendering React component"**. Pre-existing and dev-only: it is the inline consent script and the JSON-LD `<script>` in `app/layout.tsx`. Both execute from the server-rendered HTML, so production is unaffected. Do not "fix" it by moving them into `next/script`, that would put the consent defaults after gtag.js.
- **`next-env.d.ts` flips** between `.next/types` (after build) and `.next/dev/types` (after dev). Revert it before every commit; it is generated.
- **Review pipeline shape that worked:** eight lens reviewers over the code, three refuters per finding. First pass: 59 raw, 50 unique, 0 refuted, so the refuters did not discriminate and every finding needed manual triage (about 30 applied, the rest were owner calls or wrong). Second pass over the resulting diff: 30 raw, 19 unique, 16 confirmed, 3 refuted, and it caught a real dead-end bug the first pass's fix had introduced. The second pass is the one that earns its cost.
